...
(function() {
    ...
    function initializeDashboard() {
        ...
        // Set customer info
        document.getElementById('userName').textContent = `Welcome, ${currentCustomer.name}`;
  ...
  async function loadOrders(filter = "all") {
    ...
    try {
        const res = await fetch("https://smartmed-dmrb.onrender.com/api/orders/my-orders", {
            method: "GET",
            headers: {
    ...
    try {
        const response = await fetch("https://smartmed-dmrb.onrender.com/api/orders/create", {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${token}`,
...
function initializeDashboard() {
    ...
    // Initialize address form with current address
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (response.ok) {
      // ✅ Display customer name from database
      document.getElementById('customerName').textContent = data.name || 'Customer';
    } else {
